...
#   jmp2 files <slug>                list files in the live version
#   jmp2 cat <slug> <path> [ver]     print one file (ver: a number or 'staged')
...
case "$cmd" in
  ...
  files)    req GET "/sites/\${1:?usage: jmp2 files <slug>}/files" ;;
  cat)
    slug="\${1:?usage: jmp2 cat <slug> <path> [version]}"; path="\${2:?usage: jmp2 cat <slug> <path> [version]}"
    query=""; [[ -n "\${3:-}" ]] && query="?version=$3"
    req GET "/sites/$slug/files/$path$query"
    ;;
